...
public class MyAccountController implements Observer<PrietenieEntityChangeEvent> {
    ...
    public void handleOpenMessages(ActionEvent actionEvent) {
        try{
            URL fxmlLocation = HelloApplication.class.getResource("views/AllMessagesView.fxml");
            FXMLLoader fxmlLoader = new FXMLLoader(fxmlLocation);
            Parent root = (Parent) fxmlLoader.load();
            Stage stage = new Stage();
            stage.setScene(new Scene(root));
            stage.setTitle("My Messages Page");
            MessageTaskController messageTaskController = fxmlLoader.getController();
            messageTaskController.setUserCurent(userCurent);
            messageTaskController.setUtilizatorService(service);
            stage.show();
            //((Node)(actionEvent.getSource())).getScene().getWindow().hide();
        } catch(Exception e) {
            e.printStackTrace();
