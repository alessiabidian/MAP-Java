...
public class Service implements Observable<PrietenieEntityChangeEvent> {
    ...
    Repository<Long, MessageTask> repoMessageTask;

    private List<Observer<PrietenieEntityChangeEvent>> obs = new ArrayList<>();

    public Service(Repository<Long, Utilizator> repoUtilizator, Repository<String, Prietenie> repoPrietenie, Repository<Long, MessageTask> repoMessageTask){
        this.repoUtilizator = repoUtilizator;
        this.repoPrietenie = repoPrietenie;
        this.repoMessageTask = repoMessageTask;
    }

    public boolean addUser(String nume, String prenume, String email, String parola) throws ValidationException {
        Long ID = getMaxID() + 1;
        Utilizator user = new Utilizator(ID, nume, prenume, email, parola);//, dataNasterii);
        //try {
            return repoUtilizator.save(user) == null;
        /*    return true;
        }
    ...
    public boolean updateUser(Long ID, String nume, String prenume, String email, String parola) throws ValidationException {
        Utilizator user = new Utilizator(ID, nume, prenume, email, parola);//, dataNasterii);
        //try {
    ...
    public boolean updateFriendship(Long ID1, Long ID2, boolean acceptat) throws ValidationException {
        Prietenie prietenie = new Prietenie(ID1, ID2, generateCurrentDate());
    ...
    public boolean addFriendship(Long ID1, Long ID2) throws ValidationException {
        LocalDateTime friendsFrom = generateCurrentDate();
        ...
        //try {
            return repoPrietenie.save(friendship) == null;
        /*    return true;
        }
        catch (ValidationException e) {
            System.out.println(e.getMessage());
//            throw e;
    ...
    public boolean sendFriendRequest(Long ID1, Long ID2) throws ValidationException {
        LocalDateTime requestFrom = generateCurrentDate();
        ...
        if(p_dejaExistent!=null) {
            if (Objects.equals(p_dejaExistent.getID1(), ID1)  && !p_dejaExistent.isAcceptat()) //daca ID1 o mai trimis inca o data cerere
                throw new ValidationException("Cerere trimisa deja!");
            else if (Objects.equals(p_dejaExistent.getID2(), ID1)) {
                updateFriendship(ID1, ID2, true);
                return false;
            }
            else throw new ValidationException("Prietenie existenta deja!");
        }
        else {
    ...
    public boolean estePrieten(Utilizator userCurent, Utilizator user)
    {
        List<Utilizator> prieteni = getFriendsOfUserList(userCurent);
        for(Utilizator pr: prieteni)
            if(pr == user)
                return true;
        return false;
    }
    ...
    public List<Utilizator> getSentByMeRequestsUserList(Utilizator userCurent){
        List<Prietenie> allFriends = getListPrietenie();
        List<Utilizator> rezultat = new ArrayList<>();
    ...
    //------------------------------------------------------------------------------------------------------------------
    public MessageTask addMessageTask(MessageTask messageTask) throws ValidationException {
        MessageTask task = repoMessageTask.save(messageTask);
//        if(task == null) {
//            notify(new MessageTaskChangeEvent(ChangeEventType.ADD,task));
//        }
        return task;
    }

    public MessageTask deleteMessageTask(MessageTask t){
        MessageTask task = repoMessageTask.delete(t.getID());
//        if(task!=null) {
...
//        }
    ...
    public MessageTask updateMessageTask(MessageTask newTask) throws ValidationException {
        MessageTask oldTask=repoMessageTask.findOne(newTask.getID());
        if(oldTask!=null) {
            MessageTask res=repoMessageTask.update(newTask);
    ...
    public Iterable<MessageTask> getAllMessages(){
        return repoMessageTask.findAll();
    }

    public List<MessageTask> getListMessages(){
        return repoMessageTask.getAllAsList();
    }

    public List<MessageTask> getAllMessagesToFromUser(Utilizator userCurent){
        List<MessageTask> messages = new ArrayList<>();
        List<MessageTask> all = repoMessageTask.getAllAsList();
        for(MessageTask msg : all)
        {
            if(Objects.equals(msg.getTo(), userCurent.getID()) || Objects.equals(msg.getFrom(), userCurent.getID()))
                messages.add(msg);
        }
        return messages;
    }

    public List<Pair<Pair<Utilizator, Utilizator>,MessageTask>> getAllMessagesToFromUser2(Utilizator userCurent){
        List<Pair<Pair<Utilizator, Utilizator>,MessageTask>> messages = new ArrayList<>();
        List<MessageTask> all = repoMessageTask.getAllAsList();
        Pair<Utilizator, Utilizator> uu;
        Pair<Pair<Utilizator, Utilizator>,MessageTask> uum;
        for(MessageTask msg : all)
        {
            if(Objects.equals(msg.getTo(), userCurent.getID()) || Objects.equals(msg.getFrom(), userCurent.getID())) {
                Utilizator u1 = findUser(msg.getFrom());
                Utilizator u2 = findUser(msg.getTo());
                uu= new Pair<>(u1, u2);
                uum=new Pair<>(uu, msg);
                messages.add(uum);
            }
